using B2BProje.Business.Abstract;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace B2BProje.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class HavaleController : ControllerBase
    {
        IHavaleService _havaleService;

        public HavaleController(IHavaleService havaleService)
        {
            _havaleService = havaleService;
        }
        [HttpGet]
        public IActionResult GetAll()
        {
            try
            {
                // Banka listesini Business katmanından al
                var havale = _havaleService.GetAll();

                // Dönen veriyi kullanarak HTTP 200 OK durumunu döndür
                return Ok(havale);
            }
            catch (Exception ex)
            {
                // Hata durumunda 500 Internal Server Error döndür
                return StatusCode(500, $"Internal Server Error: {ex.Message}");
            }
        }
        [HttpGet("with")]
        public IActionResult GetwithAll()
        {
            try
            {
                // Banka listesini Business katmanından al
                var havale = _havaleService.GetwithAll();

                // Dönen veriyi kullanarak HTTP 200 OK durumunu döndür
                return Ok(havale);
            }
            catch (Exception ex)
            {
                // Hata durumunda 500 Internal Server Error döndür
                return StatusCode(500, $"Internal Server Error: {ex.Message}");
            }
        }
    }
}
